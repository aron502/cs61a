...
def accuracy(typed, source):
    ...
    if m == 0:
        return 0.0
    return sum(1 for t, s in zip(typed_words, source_words) if t == s) / m * 100
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    times = []
    n = len(timestamps_per_player[0])
    for timestamp in timestamps_per_player:
        tmp = [timestamp[i] - timestamp[i - 1] for i in range(1, n)]
        times.append(tmp)
...
def fastest_words(match):
    ...
    for i in word_indices:
        tmp = map(lambda p: time(match, p, i), player_indices)
        index, _ = min(
            list(enumerate(tmp)),
            key=lambda x: x[1]
        )
...
def match(words, times):
    ...
    Arguments:
        ...
        times: A list of lists for how long it took for each player to type
            each word.
            times[i][j] = time it took  player i to type words[j].
