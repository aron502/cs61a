...
def roll_dice(num_rolls, dice=six_sided):
    ...
    lst = [dice() for _ in range(num_rolls)]
    return sum(lst) if 1 not in lst else 1
...
def play(strategy0, strategy1, update,
    ...
    def play_aux(who, current, opponent):
        if current >= goal or opponent >= goal:
            return (current, opponent) if who == 0 else (opponent, current)
        strategy = strategy0 if who == 0 else strategy1
        current = update(strategy(current, opponent), current, opponent, dice)
        return play_aux(1 - who, opponent, current)

    return play_aux(who, score0, score1)
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    val = 0
    for i in range(1, 11):
        tmp = make_averaged(roll_dice, samples_count)(i, dice)
        if val < tmp:
            val = tmp
            n = i
    return n
